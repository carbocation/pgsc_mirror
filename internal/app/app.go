...
func New(ctx context.Context, cfg config.Config, writable bool) (*App, error) {
	...
	if cfg.Targets.GCS {
		gs, err := gcsstore.New(ctx, cfg.GCS.Bucket, cfg.GCS.Prefix, cfg.GCS.BillingProject)
		if err != nil {
			return nil, err
		}
...
func (a *App) inventory(ctx context.Context, previous []model.Entry, allowLimit bool) (inventory, error) {
	...
	for n := 0; n < workers; n++ {
		...
		go func() {
			...
			for i := range jobs {
				...
				first := now
				e := model.Entry{PGSID: id, GenomeBuild: a.Config.GenomeBuild, SourceURL: sourceURL, SourceMD5: sum, BlobKey: model.BlobKey(sum), FirstSeenAt: first, LastSeenAt: now, Status: model.StatusReady, License: licenses[id]}
	...
	return inventory{ids: ids, totalIDs: totalIDs, truncated: truncated, metadata: metadataDoc.Body, entries: entries, scoreDoc: scoreDoc, metadataDoc: metadataDoc}, nil
}
...
func (a *App) Plan(ctx context.Context) (PlanReport, error) {
	...
	inv, err := a.inventory(ctx, previous, true)
	if err != nil {
		return PlanReport{}, err
	}
...
func (a *App) reconcile(ctx context.Context, command string) (report RunReport, runErr error) {
	...
	defer func() {
		if err := a.releaseLease(context.Background(), lease); err != nil && runErr == nil {
			runErr = err
		}
	}()
	p, previous, err := a.latest(ctx)
	...
	inv, err := a.inventory(ctx, previous, false)
	if err != nil {
		return report, err
	}
	changes := planner.Plan(previous, inv.entries)
	...
	if err := a.ensureBlobs(ctx, inv.entries); err != nil {
		return report, err
	}
	now := a.now().UTC()
...
func (a *App) acquireLease(ctx context.Context) (heldLease, error) {
	if len(a.targets) == 0 {
		return heldLease{}, errors.New("no target for lease")
	}
	t := a.targets[0]
	host, _ := os.Hostname()
	now := a.now().UTC()
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if err == nil {
			return heldLease{target: t, generation: info.Generation}, nil
		}
		...
		if openErr != nil {
			return heldLease{}, openErr
		}
		var old model.Lease
		decodeErr := json.NewDecoder(io.LimitReader(r, 1<<20)).Decode(&old)
		r.Close()
		if decodeErr != nil || old.ExpiresAt.After(now) {
			return heldLease{}, fmt.Errorf("synchronization lease is held by %s until %s", old.Owner, old.ExpiresAt.Format(time.RFC3339))
		}
		gen := oldInfo.Generation
		if err := t.Delete(ctx, model.LeaseKey, store.DeleteOptions{GenerationMatch: &gen}); err != nil {
			return heldLease{}, fmt.Errorf("remove expired lease: %w", err)
		}
	}
	return heldLease{}, errors.New("could not acquire synchronization lease")
}
