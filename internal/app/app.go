...
func requireCurrentScoreLayout(pointer model.Pointer, entries []model.Entry) error {
	if pointer.ScoreLayoutVersion != model.ScoreLayoutVersion {
		return fmt.Errorf("release %s has unsupported score layout version %d; want %d", pointer.ReleaseID, pointer.ScoreLayoutVersion, model.ScoreLayoutVersion)
	}
	for i := range entries {
		if entries[i].ReleaseID != pointer.ReleaseID {
			return fmt.Errorf("release %s entry %s belongs to release %s", pointer.ReleaseID, entries[i].PGSID, entries[i].ReleaseID)
		}
		want := model.ScoreKey(entries[i].PGSID, entries[i].GenomeBuild)
		if entries[i].ScoreKey != want {
...
func (a *App) reconcile(ctx context.Context, command string) (report RunReport, runErr error) {
	...
	if err := a.publish(ctx, pointer, inv.scoreDoc.Body, inv.metadata, manifestBytes, manifestTSV); err != nil {
		return report, err
	}
	if err := a.State.RecordRelease(ctx, pointer, inv.entries); err != nil {
		return report, err
	}
	if err := a.recordSentinels(ctx, inv); err != nil {
		return report, err
